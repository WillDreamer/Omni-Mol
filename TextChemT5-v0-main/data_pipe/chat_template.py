...
def preprocess_phi(
    ...
    # print('sep', sep)
    for conversation, target in zip(conversations, targets):
        # NOTE: Since Phi-3 small use the same special toke for 
    ...
    # NOTE: Checked, compatible with Phi-3 small
...
def preprocess_t5(
    ...
    for i, source in enumerate(sources):
        ...
        assert len(source) == 2, "T5 expects exactly one turn of conversation"
        assert source[0]["from"] == "human" and source[1]["from"] == "gpt"
        
        human_msg = source[0]["value"]  # message from human --> question
        assistant_msg = source[1]["value"]  # message from assistant --> answer
        
        task_prefix = "Analyze the molecule: "
        
        if has_image and "<image>" in human_msg:
            human_msg = human_msg.replace("<image>", "<extra_id_0>")
            
        inputs_list.append(task_prefix + human_msg)
        targets_list.append(assistant_msg)
    
    if has_image:
        input_ids_list = []
        for inp in inputs_list:
            tokens = tokenizer(inp, return_tensors="pt", add_special_tokens=True).input_ids[0]
            
            extra_id = tokenizer.convert_tokens_to_ids("<extra_id_0>")
            tokens[tokens == extra_id] = -200
        ...
        attention_mask = model_inputs.attention_mask  # all ones
    ...
    return dict(
        ...
        decoder_attention_mask=(labels != IGNORE_INDEX).long()
    )
...
def apply_chat_template(
    ...
    elif conversation_lib.default_conversation.version == "t5":
        return preprocess_t5(messages, tokenizer, has_image)
    else:

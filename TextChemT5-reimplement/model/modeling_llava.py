...
class GraphLlavaForConditionalGeneration(LlavaPreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if graphs is None or (graphs is not None and all(g is None for g in graphs)):
            return None, labels, input_ids, attention_mask, past_key_values, None
        ...
        for batch_idx in range(input_ids.shape[0]):
            ...
            if len(mm_pos) == 0:
                ...
                fused_embeds[batch_idx, :input_ids[batch_idx].shape[0], :] = self.language_model.encoder.embed_tokens(input_ids[batch_idx])
                if labels is not None:

...
def create_t5_model(model_args: ModelArguments, training_args: TrainingArguments) -> tuple[PreTrainedTokenizer, PreTrainedModel]:
    """Create T5 model for stage 2 training
    ...
    Returns:
        tuple[PreTrainedTokenizer, PreTrainedModel]: tokenizer for T5 and the model itself
    """
    # 1. Init all configs
    graph_config = GraphConfig(
        model_name=model_args.graph_tower,
    ...
    text_config = T5Config.from_pretrained(
        model_args.base_model,  # should be "t5-base"
        torch_dtype=torch.bfloat16
    )
    ...
    # Important: specify language_backbone_name as "t5"
    config = GraphLlavaConfig(
        ...
        projector_config=projector_config,
        moe_enable=False,
        language_backbone_name="t5",  # This is crucial for T5
        enable_apple_loss=model_args.enable_apple_loss
    )
    config.use_cache = False
    text_config.use_cache = False
    ...
    tokenizer = T5Tokenizer.from_pretrained(model_args.base_model)
    tokenizer.model_max_length = training_args.model_max_length
    
    # Import the T5 graph model
    from model.modeling_t5_graph import GraphT5ForConditionalGeneration
    
    with no_init_weights():
        model = GraphT5ForConditionalGeneration(config)
    
    # 3. Load pre-trained components with weight loading statistics
    logger.info("Loading T5 pretrained weights...", on_rank0=True)
    
    # Get the state dict of our model's language_model
    model_state_dict = model.language_model.state_dict()
    total_params = len(model_state_dict)
    
    # Load pretrained T5 model
    from transformers import T5ForConditionalGeneration
    pretrained_t5 = T5ForConditionalGeneration.from_pretrained(
        model_args.base_model,
        config=text_config,
        torch_dtype=torch.bfloat16
    )
    pretrained_state_dict = pretrained_t5.state_dict()
    ...
    for key, param in model_state_dict.items():
        if key in pretrained_state_dict:
            ...
            if param.shape == pretrained_param.shape:
                ...
                mismatched_keys.append(f"{key}: expected shape {param.shape}, got {pretrained_param.shape}")
        else:
            missing_keys.append(key)
    
    # Check for unexpected keys in pretrained model
    for key in pretrained_state_dict:
        if key not in model_state_dict:
            unexpected_keys.append(key)
    
    # Calculate and display statistics
    load_percentage = (loaded_params / total_params) * 100 if total_params > 0 else 0
    
    logger.info(f"\n{'='*50}", on_rank0=True)
    logger.info(f"T5 Weight Loading Statistics:", on_rank0=True)
    logger.info(f"{'='*50}", on_rank0=True)
    logger.info(f"Total parameters in model: {total_params}", on_rank0=True)
    logger.info(f"Successfully loaded: {loaded_params} ({load_percentage:.2f}%)", on_rank0=True)
    logger.info(f"Missing keys: {len(missing_keys)}", on_rank0=True)
    logger.info(f"Unexpected keys: {len(unexpected_keys)}", on_rank0=True)
    logger.info(f"Mismatched shapes: {len(mismatched_keys)}", on_rank0=True)
    
    if missing_keys and len(missing_keys) <= 10:  # Show first 10 missing keys
        logger.info(f"\nFirst few missing keys:", on_rank0=True)
        for key in missing_keys[:10]:
            logger.info(f"  - {key}", on_rank0=True)
    elif missing_keys:
        logger.info(f"\nShowing first 10 of {len(missing_keys)} missing keys...", on_rank0=True)
        for key in missing_keys[:10]:
            logger.info(f"  - {key}", on_rank0=True)
    
    if mismatched_keys:
        logger.info(f"\nShape mismatches:", on_rank0=True)
        for mismatch in mismatched_keys:
            logger.info(f"  - {mismatch}", on_rank0=True)
    
    logger.info(f"{'='*50}\n", on_rank0=True)
    
    # Update model's language_model with loaded weights
    model.language_model.load_state_dict(model_state_dict, strict=False)
    
    # Clean up pretrained model
    del pretrained_t5
    
    # Load projector (if pretrained)
    ...
    # Activate T5 model (no LoRA, full fine-tuning)
    model.language_model.requires_grad_(True)
    
    # Optionally freeze graph tower to save memory
    model.graph_tower.requires_grad_(False)
...
def load_t5_model(
    ...
    task_embed_path: str = None,
    **kwargs
) -> tuple[PreTrainedTokenizer, GraphT5ForConditionalGeneration]:
    """Load T5 model for inference
    
    Args:
        model_path (str): path to the saved T5 model
        language_backbone (str): path to T5 backbone (e.g., "t5-base")
        graph_path (str): path to graph checkpoint
        use_flash_attn (bool): Whether to use flash attention
        
    Returns:
        tuple[PreTrainedTokenizer, GraphT5ForConditionalGeneration]: tokenizer and model
    """
    from model.modeling_t5_graph import GraphT5ForConditionalGeneration
    from transformers import T5Tokenizer
    ...
    config = GraphLlavaConfig.from_pretrained(model_path)
    if use_flash_attn:
        # T5 doesn't support flash attention in the same way as decoder-only models
        logger.warning("Flash attention is not fully supported for T5 models")
    ...
    # 3. Load tokenizer
    tokenizer = T5Tokenizer.from_pretrained(language_backbone)
    
    # 4. Load weights from model.safetensors
    logger.info('Loading T5 model weights from safetensors...', on_rank0=True)
    ...
    # Check for model.safetensors in the model_path directory
    safetensors_files = []
    ...
    if not safetensors_files:
        raise FileNotFoundError(f"No model.safetensors files found in {model_path}")
    
    # Load all safetensors files
    loaded_state_dict = {}
    for safetensor_file in sorted(safetensors_files):
        safetensor_path = os.path.join(model_path, safetensor_file)
        logger.info(f"Loading {safetensor_file}...", on_rank0=True)
        partial_state_dict = load_file(safetensor_path)
        loaded_state_dict.update(partial_state_dict)
    
    # Log loaded keys info
    logger.info(f"Total keys in safetensors: {len(loaded_state_dict)}", on_rank0=True)
    logger.info(f"Total keys in model: {total_params}", on_rank0=True)
    ...
    for key, param in model_state_dict.items():
        if key in loaded_state_dict:
            ...
            if param.shape == loaded_param.shape:
                ...
                mismatched_keys.append(f"{key}: expected shape {param.shape}, got {loaded_param.shape}")
        else:
            missing_keys.append(key)
    
    # Check for unexpected keys in loaded state dict
    ...
    logger.info(f"\n{'='*60}", on_rank0=True)
    logger.info(f"T5 Model Loading Statistics:", on_rank0=True)
    logger.info(f"{'='*60}", on_rank0=True)
    logger.info(f"Total parameters in model: {total_params}", on_rank0=True)
    logger.info(f"Successfully loaded: {loaded_params} ({load_percentage:.2f}%)", on_rank0=True)
    logger.info(f"Missing keys: {len(missing_keys)}", on_rank0=True)
    logger.info(f"Unexpected keys: {len(unexpected_keys)}", on_rank0=True)
    ...
    # Show detailed information if needed
    if missing_keys:
        logger.info(f"\nFirst 10 missing keys:", on_rank0=True)
        for key in missing_keys[:10]:
            logger.info(f"  - {key}", on_rank0=True)
        if len(missing_keys) > 10:
            logger.info(f"  ... and {len(missing_keys) - 10} more", on_rank0=True)
    
    if unexpected_keys:
        logger.info(f"\nFirst 10 unexpected keys:", on_rank0=True)
        for key in unexpected_keys[:10]:
            logger.info(f"  - {key}", on_rank0=True)
        if len(unexpected_keys) > 10:
            logger.info(f"  ... and {len(unexpected_keys) - 10} more", on_rank0=True)
    ...
    # Categorize missing keys by module
    if missing_keys:
        missing_by_module = {}
        for key in missing_keys:
            module = key.split('.')[0]
            if module not in missing_by_module:
                missing_by_module[module] = 0
            missing_by_module[module] += 1
        
        logger.info(f"\nMissing keys by module:", on_rank0=True)
        for module, count in missing_by_module.items():
            logger.info(f"  - {module}: {count} keys", on_rank0=True)
    ...
    # 5. Load graph encoder if provided
    if graph_path and os.path.exists(graph_path):
        logger.info(f"Loading graph encoder from {graph_path}", on_rank0=True)
        try:
            model.load_graph(graph_path)
            logger.info("Graph encoder loaded successfully!", on_rank0=True)
        except Exception as e:
            logger.warning(f"Failed to load graph encoder: {e}", on_rank0=True)
    
    # 6. Load task embedding if provided
    if task_embed_path and os.path.exists(task_embed_path):
        logger.info(f"Loading task embedding from {task_embed_path}", on_rank0=True)
        try:
            model.load_task_embed(task_embed_path)
            logger.info("Task embedding loaded successfully!", on_rank0=True)
        except Exception as e:
            logger.warning(f"Failed to load task embedding: {e}", on_rank0=True)
    
    # Final success message
    if load_percentage >= 90:
        logger.info(f'✅ T5 model loaded successfully with {load_percentage:.2f}% parameters!', on_rank0=True)
    elif load_percentage >= 70:
        logger.warning(f'⚠️  T5 model loaded with {load_percentage:.2f}% parameters. Some components may be missing.', on_rank0=True)
    else:
        logger.error(f'❌ T5 model loaded with only {load_percentage:.2f}% parameters. This may cause issues!', on_rank0=True)

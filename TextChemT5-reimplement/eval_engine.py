from model_factory import load_lora_model, load_moe_lora_model,load_moe_lora_model_sequential,load_partial_model, load_t5_model
...
def distributed_eval(loader, model, generation_config, tokenizer, args):
    all_batches = []
    for data in loader:
    ...
    # 检查是否是T5模型
    is_t5 = hasattr(model.config, 'text_config') and model.config.text_config.is_encoder_decoder
    ...
    with accelerator.split_between_processes(all_batches) as batch_list:
        pbar = tqdm(total=len(batch_list), desc=f"[rank{local_rank}]")
        for batch in batch_list:
            ...
            gen_kwargs = {
                ...
                "use_cache": True,
                "this_task_ids": batch["this_task_ids"].to(args.device) if "this_task_ids" in batch else None
            }
            ...
            # 解码输出
            for i, (result, input_id, prompt, gt) in enumerate(zip(output_ids, input_ids, batch["prompt"], batch["gt"])):
                if is_t5:
                    # T5生成的是完整输出
                    pred_text = tokenizer.decode(result, skip_special_tokens=True)
                else:
                    # decoder-only模型需要截取
                    pred_text = tokenizer.decode(result[input_id.shape[0]:], skip_special_tokens=True)
...
def vanilla_eval(loader, model, generation_config, tokenizer, args):
    output = []
    ...
    # 确保T5的配置正确
    if is_t5:
        # T5使用pad_token_id作为decoder_start_token_id
        if generation_config.decoder_start_token_id is None:
            generation_config.decoder_start_token_id = tokenizer.pad_token_id
    ...
    for idx, batch in enumerate(tqdm(loader, desc="Evaluating")):
        ...
        # 解码输出
        for i, (result, prompt, gt) in enumerate(zip(output_ids, batch["prompt"], batch["gt"])):
            if is_t5:
                # T5生成的是完整输出，不需要截取
                pred_text = tokenizer.decode(result, skip_special_tokens=True)
            else:
                # 对于decoder-only模型，需要截取输入部分
                pred_text = tokenizer.decode(result[input_ids[i].shape[0]:], skip_special_tokens=True)
